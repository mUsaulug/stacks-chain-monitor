...
COMMENT ON TABLE notification_dead_letter_queue IS
    ...
    'Tracks notifications that exceeded max retry attempts or encountered permanent failures. ' ||
    'Enables manual investigation and re-processing by admins.';

COMMENT ON COLUMN notification_dead_letter_queue.failure_reason IS
    'Why notification failed permanently: CIRCUIT_OPEN (circuit breaker triggered), ' ||
    'MAX_RETRIES_EXCEEDED (all retry attempts exhausted), TIMEOUT (delivery timeout), ' ||
    'INVALID_RECIPIENT (email/webhook invalid), etc.';
...
COMMENT ON COLUMN alert_notification.delivery_status IS
    ...
    'PENDING (not yet sent), DELIVERING (in progress), DELIVERED (success), ' ||
    'RETRYING (transient failure, will retry), DEAD_LETTER (permanent failure, in DLQ)';
...
COMMENT ON INDEX idx_dlq_pending IS
    'Partial index: Fast queries for unprocessed DLQ items (admin dashboard). ' ||
    'Only indexes rows where processed = FALSE.';

COMMENT ON INDEX idx_notification_retry_queue IS
    'Partial index: Fast processing of retry queue (ordered by last attempt time). ' ||
    'Used by scheduled job to retry RETRYING notifications after backoff period.';
